<?php
	error_reporting(0);
	$page = $_GET['page'];
	if(isset($_POST['save_settings'])) { leagueengine_save_settings(); }
?>
...
<div id="leagueengine_admin" class="<?php echo $page; ?>">
	...
	<div id="leagueengine_admin_content">
		<h2 class="form_title"><?php _e('Import','leagueengine'); ?></h2>
...
<?php
    ...
    $importdir  = $upload_dir['basedir']."/imported_csv/";
		...
		<table class="form" style="margin-bottom:40px;">
			...
			<input style="margin-top:20px;" type='submit' name= 'post_csv' id='post_csv' class="button" value='Import CSV File' />
		</form>
 ...
 <div style="float:left;min-width:45%">
     ...
     <p style="color:#FF0000" id="drop_table">  <label><input name="drop_table" type="checkbox"  value="drop_table" /><strong> (TRUNCATE TABLE) Delete all the data before import </strong></label> </p>
		...
		<div id="where" style="display:none">	
			...
			<input style="margin-top:20px;" type='submit' name= 'post_csv' id='post_csv' value='Import' />
		</form>
...
// Code modified at version 1.1.2
	...
	?>
		...
		<table class="form">
			...
			<tr>
				<td><?php _e('CSV File','leagueengine');?></td>
				<td>
					<input name="importop" id="importop" type="hidden" value="CSV_file" />
			...
			<tr>
				...
				<td>
					<select class="select2-nosearch" name="tabella" id="tabella" aria-required="true">
		            <?php
					global $wpdb;
			...
			<tr>
				<td><?php _e('Delimiter','leagueengine');?></td>
				<td>
				    <select class="select2-nosearch" name="delim" id="delim">
					<option value=",">,</option>
					<option value=";">;</option>
				    </select>
			    </td>
		...
		<input style="margin-top:20px;" type="submit" name="Import" class="button" value="<?php _e('Next','leagueengine');?>" />
