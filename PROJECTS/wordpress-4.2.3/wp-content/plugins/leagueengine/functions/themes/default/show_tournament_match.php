...
// SHORTCODE: LEAGUEENGINE_SHOW_TOURNAMENT_MATCH
// DESCRIPTION: DISPLAYS SINGLE MATCH FROM TOURNAMENT

add_shortcode('leagueengine_show_tournament_match', 'leagueengine_show_tournament_match');

function leagueengine_show_tournament_match($atts, $content = null) {
	...
	ob_start();
	$output = '';
	$output .= '<div class="leagueengine show_tournament_match '.$class.'">';

	$table = $wpdb->prefix . 'leagueengine_tournament_matches';
	$match = $wpdb->get_row("SELECT * FROM $table WHERE tournament_id = '$tid' AND id = '$mid'");
	...
	if($match) {
		...
		$output .= '<table>';
			...
			$output .= '</tr>';
			$output .= '<tr>';
				$output .= '<td class="home_team_text" style="text-align:center;width:40%;"><h3>'. leagueengine_link('team&tid='.$match->home_team_id,leagueengine_fetch_data_from_id($match->home_team_id,'data_value')) . '</td>';
				$output .= '<td style="width:10%;"></td>';
				$output .= '<td class="away_team_text" style="text-align:center;width:40%;"><h3>'. leagueengine_link('team&tid='.$match->away_team_id,leagueengine_fetch_data_from_id($match->away_team_id,'data_value')) . '</td>';
			$output .= '</tr>';
		...
		if(current_user_can('leagueengine') or current_user_can('manage_options')) {
			$output .= '<tr><td colspan="100%" style="text-align:center;"><a href="wp-admin/admin.php?page=leagueengine_tournament_match&tid='.$tid.'&mid='.$mid.'">'.__('Edit Match','leagueengine').'</a></td></tr>';
		}
		...
		foreach($results as $result) {
			...
			if($result->col == 'attributes') {
				...
				if(leagueengine_match_has_attributes(NULL,NULL,$tid,$mid)) {
				if($attributes_title) { $output .= '<h3 class="attributes">'.$attributes_title.'</h3>'; }
				$table = $wpdb->prefix . 'leagueengine_data';
				$attributes = $wpdb->get_results("SELECT * FROM $table WHERE data_assign = 'tournament_match' AND att_type != 'statistic' ORDER BY sort_order ASC, data_value ASC");
				if($attributes) {
					$output .= '<table class="tournament_match_attributes">';
					foreach ($attributes as $attribute) {
						if(leagueengine_fetch_tournament_match_attribute($tid,$mid,$attribute->id)) {
						    $output .= '<tr>';
							$output .= '<td style="width:50%; text-align:right;">'.leagueengine_fetch_data_from_id($attribute->id,'data_value').'</td>';    
							$output .= '<td></td>';    
							$output .= '<td style="width:50%; text-align:left;">'.leagueengine_fetch_tournament_match_attribute($tid,$mid,$attribute->id).'</td>';    
						    $output .= '</tr>';
						}
					}
					$output .= '</table>';
			...
			if($result->col == 'summary') {
				...
				$home_team_id = leagueengine_match_info('tournament_match',NULL,NULL,$tid,$mid,'home_team_id');
				$away_team_id = leagueengine_match_info('tournament_match',NULL,NULL,$tid,$mid,'away_team_id');
		
				$events = $wpdb->get_results("SELECT * FROM $table WHERE tournament_id = '$tid' AND match_id = '$mid' AND event_id <> 'app' AND event_id <> 'sub' GROUP BY event_id ORDER by sort_order ASC");

				if($events) {
				$output .= '<table class="tournament_match_summary" style="float:left;width:50%;">';

				foreach($events as $event) {
					$event_id = $event->event_id;
					$event_players = $wpdb->get_results("SELECT * FROM $table WHERE tournament_id = '$tid' AND match_id = '$mid' AND event_id = '$event_id' AND team_id = '$home_team_id' GROUP BY player_id");
					
					foreach($event_players as $event) {
						$event_id = $event->event_id;
						$player_id = $event->player_id;
						$count = $wpdb->get_var("SELECT count(*) FROM $table WHERE tournament_id = '$tid' AND match_id = '$mid' AND event_id = '$event_id' AND player_id = '$player_id'");
						...
						$output .= '<tr>';
							...
							$output .= '<td style="width:35%;text-align:left;">'.leagueengine_link('player&pid='.$event->player_id.'&tid='.$tid,leagueengine_fetch_data_from_id($event->player_id,'data_value')).'</td>';
						$output .= '</tr>';
					}
				...
				foreach($events as $event) {
					$event_id = $event->event_id;
					$event_players = $wpdb->get_results("SELECT * FROM $table WHERE tournament_id = '$tid' AND match_id = '$mid' AND event_id = '$event_id' AND team_id = '$away_team_id' GROUP BY player_id");
					...
					foreach($event_players as $event) {
						...
						if($count > 1) { $count = '<span> x' . $count . '</span>'; } else { $count = ''; } 
						$output .= '<tr>';
							$output .= '<td style="width:35%;text-align:right;">'.leagueengine_link('player&pid='.$event->player_id.'&tid='.$tid,leagueengine_fetch_data_from_id($event->player_id,'data_value')).'</td>';
							$output .= '<td style="width:15%;text-align:center;"><img  width="16" height="16" src="'.leagueengine_fetch_data_from_id($event->event_id,'image').'" />'.$count.'</td>';
						$output .= '</tr>';
					}
			...
			/***************************************************************/	
				...
				if($result->col == 'preview') {
					if (strtotime($match->match_date . ' ' .$match->match_time) > time()) {
						...
						$table = $wpdb->prefix . 'leagueengine_tournament_matches';
						$preview = $wpdb->get_var("SELECT preview FROM $table WHERE tournament_id = '$tid' AND id = '$mid'");
						if($preview) {
							$output .= '<div class="tournament_match_preview">';
			...
			elseif($result->col == 'lineups') {
				...
				$home_team_id = leagueengine_match_info('tournament_match',NULL,NULL,$tid,$mid,'home_team_id');
				$away_team_id = leagueengine_match_info('tournament_match',NULL,NULL,$tid,$mid,'away_team_id');
				$homeplayers = $wpdb->get_results("SELECT player_id FROM $table WHERE tournament_id = '$tid' AND match_id = '$mid' AND team_id = '$home_team_id' AND event_id = 'app'");
				$awayplayers = $wpdb->get_results("SELECT * FROM $table WHERE tournament_id = '$tid' AND match_id = '$mid' AND team_id = '$away_team_id'  AND event_id = 'app'");
				$homesubs = $wpdb->get_results("SELECT * FROM $table WHERE tournament_id = '$tid' AND match_id = '$mid' AND team_id = '$home_team_id'  AND event_id = 'sub'");
				$awaysubs = $wpdb->get_results("SELECT * FROM $table WHERE tournament_id = '$tid' AND match_id = '$mid' AND team_id = '$away_team_id'  AND event_id = 'sub'");
				
				if($homeplayers or $awayplayers) {
					if($lineups_title) { $output .= '<h3 class="lineups">'.$lineups_title.'</h3>'; }
					$output .= '<table class="tournament_match_lineups">';
					$output .= '<tr>';
						$output .= '<th colspan="100%" style="text-align:center;">'.__('Lineups','leagueengine').'</th>';
					$output .= '</tr>';
					$output .= '<tr>';
						$output .= '<td style="text-align:center;width:50%;">';
						$i = 1; foreach ($homeplayers as $homeplayer) {
							$output .= leagueengine_link('player&pid='.$homeplayer->player_id.'&tid='.$tid,leagueengine_fetch_data_from_id($homeplayer->player_id,'data_value')).'<br/>';
							$i++;
						}
						$output .= '</td>';
						...
						$i = 1; foreach ($awayplayers as $awayplayer) {
							$output .= leagueengine_link('player&pid='.$awayplayer->player_id.'&tid='.$tid,leagueengine_fetch_data_from_id($awayplayer->player_id,'data_value')).'<br/>';
							$i++;
						}
						$output .= '</td>';
					$output .= '</tr>';
				...
				if($homesubs or $awaysubs) {
					$output .= '<table class="tournament_match_subs">';
					$output .= '<tr>';
			...
			$table = $wpdb->prefix . 'leagueengine_tournament_matches_events';
			$order = leagueengine_fetch_settings('site_match_events_order');
			$events = $wpdb->get_results("SELECT * FROM $table WHERE tournament_id = '$tid' AND match_id = '$mid' AND event_id <> 'app' AND event_id <> 'sub' ORDER BY event_time + 0 $order");
				if($events) {
					if($events_title) { $output .= '<h3 class="events">'.$events_title.'</h3>'; }
					$output .= '<div class="tournament_match_events" style="float:left;width:100%;">';
					...
					foreach($events as $event) {
						...
						$output .= '<div class="row" style="float:left;width:100%;">';
							...
							if($event->player_id) {
								$output .= '<p class="event"><span class="event_name">'.leagueengine_fetch_data_from_id($event->event_id,'data_value').'</span> &mdash; <span class="event_player">'.leagueengine_link('player&pid='.$event->player_id.'&tid='.$tid,leagueengine_fetch_data_from_id($event->player_id,'data_value')).'</span></p>';
							} else {
			...
			elseif($result->col == 'statistics') {
				...
				if(leagueengine_match_has_statistics(NULL,NULL,$tid,$mid)) {

					$table = $wpdb->prefix . 'leagueengine_data';
					$statistics = $wpdb->get_results("SELECT * FROM $table WHERE data_assign = 'tournament_match' AND att_type = 'statistic' ORDER BY sort_order ASC, data_value ASC");
					if($statistics) {
						if($statistics_title) { $output .= '<h3 class="statistics">'.$statistics_title.'</h3>'; }
						$output .= '<table class="tournament_match_statistics">';
						$output .= '<tr>';
							$output .= '<th colspan="100%" style="text-align:center;">'.__('Statistics','leagueengine').'</th>';
						$output .= '</tr>';
										
						foreach ($statistics as $statistic) {
										
							$getstat = leagueengine_fetch_tournament_match_statistics($tid,$mid,$statistic->id);
							if($getstat) {
								...
								$home_stat = leagueengine_tournament_match_statistic_value($tid,$mid,$statistic->id,'home');
								$away_stat = leagueengine_tournament_match_statistic_value($tid,$mid,$statistic->id,'away');
			...
			elseif($result->col == 'report') {
			
				$table = $wpdb->prefix . 'leagueengine_tournament_matches';
				$report = $wpdb->get_var("SELECT report FROM $table WHERE tournament_id = '$tid' AND id = '$mid'");
				if($report) {
					$output .= '<div class="tournament_match_report">';
