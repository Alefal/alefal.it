...
A Calendar to show the day-to-day availability of a room/house. Integration with ContactForm 7 for on-line booking.
Synch with an External Calendar.
...
Export/Import to/from a iCal file is also possible. Only bookings information is included and each room/item needs a separate file.
Another option is to download bookings (manual or recurrent scheduled) with a link provided by the external calendar. For each room/item a separate link is required.
...
The settings are moved to the WP Availability Calendar and Booking menu. A general setting is added to control the user accesibility of this menu. iCal file export and import added. Download with a link provided by an external calendar added.
 
Version 1.2.0
Synchronisation with External Calendar added.
