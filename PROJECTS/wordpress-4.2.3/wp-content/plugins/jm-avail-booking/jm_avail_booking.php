...
function AvailabilityBooking_install() {
    ...
    $AvailabilityBooking_db_version = "3.0";
    /*
     * 
    ...
    /*
     * After adding the db field uid it must be filled according to the uid definition id-name@domain.
     * This is required for compatability reasons with airb&b
     * The new uid definition is time()-name@domain
     * 
     * 
     */
    $uid_updated = get_option("AvailabilityBooking_Uid_Updated");
    if ($uid_updated == 0) {
        ...
        foreach ($items as $item) {
            $split = explode('@', $item['email']);
            $name = $split[0];
            $uid = $item['id'] . "-" . $name . "@" . $site_name;
            $newdata = array('uid' => $uid);
            $where = array('id' => $item['id']);
            $avail_booking_db->update($newdata, $where);
        }
        add_option('AvailabilityBooking_Uid_Updated', '1');
    }
}
 ...
 * Cron schedule
 */
add_action('avail_cron_hook', 'avail_booking_scheduled_upload');

function avail_booking_scheduled_upload() {
    AvailabilityBookingFunctions::avail_booking_link_upload();
}
...
function jm_avail_booking_init() {
    ...
    $options = get_option('jm_avail_booking_main_options');
    if (isset($options['capability']))    {
        $jm_capability = $options['capability'];
    } else {
        $jm_capability = 'manage_options';
    }
}
...
function wpcf7_booking_shortcode_handler($tag) {
    ...
    if ($tag['name'] != "") {
        ...
        if ($options[rooms] != "") {
//$rooms_values 
           $rooms_values = AvailabilityBookingFunctions::avail_booking_room_names($options,'');

            if ($tag[attr] != "") {
               $rooms_names = AvailabilityBookingFunctions::avail_booking_room_names('none',$tag[attr]);
            } else {
//$rooms_names 
                $rooms_names = AvailabilityBookingFunctions::avail_booking_room_names($options,'');
            }
        } else {
...
function avail_cf7_db_update($cf7) {
    ...
    if ($submission) {
        ...
        $split = explode('@', $posted_data['your-email']);
        $name = $split[0];
        $site_name = parse_url(site_url(), PHP_URL_HOST);
        $uid = time() . "-" . $name . "@" . $site_name;
        $wpdb->insert(
...
if (is_admin()) {
    ...
    require_once ('avail_booking_settingspage.php');
}
