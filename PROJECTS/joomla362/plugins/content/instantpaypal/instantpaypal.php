...
class plgContentInstantPaypal extends JPlugin { 
	private function sendEmailNotify(&$session, &$params, &$mainframe) {
		...
		$customPriceAmount = JRequest::getVar('instantpaypalcustomamount', null);
	...
	private function runPlugin($context, &$article, &$params, $page = 0) { 
		...
		if (count ( $matches [0] )) {
			for($i = 0; $i < count ( $matches [0] ); $i ++) {
				...
				$priceAmount = $editPrice ? '<input style="width:30px" type="text" id="amountfield' . $i . '" name="amount" value="' . $price . '" />' . $this->params->get('currency_code', 'USD') : '<input type="hidden" name="amount" value="' . $price . '" />';
				...
				$jsCode = <<<JSCODE
								...
								//<![CDATA[
									var sendEmailIframe = function(eventObject, articleID, productIterationID) {
									    ...
									    iframe.setAttribute("src","index.php?option=com_content&view=article&id=" + articleID + "&articlenamespace=" + articleID + "&instantpaypaltask=sendemailnotify&instantpaypalindex=" + productIterationID + qtyAmount + customAmount + miniFormQueryString);
