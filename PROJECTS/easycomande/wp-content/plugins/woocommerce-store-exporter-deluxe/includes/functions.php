...
if( is_admin() ) {
	...
	function woo_ce_admin_fail_notices() {
		...
		// Displays HTML notice if WooCommerce Checkout Add-Ons is installed
...
/*
		// @mod - Will work on in the 2.0+ series, requires a mountain of testing before release
...
*/
	...
	// Delete all WordPress Options generated by Store Exporter Deluxe
...
function woo_ce_fatal_error() {
	...
	if( $error !== null ) {
		$message = '';
		$notice = sprintf( __( 'Refer to the following error and if you continue to have problems see our <a href="%s" target="_blank">Usage</a> document or contact us on <a href="http://www.visser.com.au/premium-support/" target="_blank">Premium Support</a> for further assistance.<br /><br /><code>%s in %s on line %d</code>', 'woocommerce-exporter' ), $troubleshooting_url, $error['message'], $error['file'], $error['line'] );
		if ( substr( $error['message'], 0, 22 ) === 'Maximum execution time' ) {
			$message = __( 'The server\'s maximum execution time is too low to complete this export. This is commonly due to a low timeout limit set by your hosting provider or PHP Safe Mode being enabled.', 'woocommerce-exporter' );
		} elseif ( substr( $error['message'], 0, 19 ) === 'Allowed memory size' ) {
			$message = __( 'The server\'s maximum memory size is too low to complete this export. Consider increasing available memory to WordPress or reducing the size of your export.', 'woocommerce-exporter' );
