	...
	if( !class_exists( 'VL_Updater' ) && !function_exists( 'vl_common_check_for_plugin_update' ) ) {
		function vl_common_check_for_plugin_update( $checked_data ) {
			...
			if( !class_exists( 'VL_Updater' ) )
				$message = __( 'Install the Visser Labs Updater from the Plugins screen to install this Plugin update.', 'woocommerce-exporter' );
	...
	if( !class_exists( 'VL_Updater' ) && !function_exists( 'vl_common_make_request' ) ) {
		function vl_common_make_request( $action, $args ) {
			...
			if( is_wp_error( $request ) )
				$response = new WP_Error( 'plugins_api_failed', __( 'An Unexpected HTTP Error occurred during the API request.</p> <p><a href="?" onclick="document.location.reload(); return false;">Try again</a>', 'woocommerce-exporter' ), $request->get_error_message() );
			else
