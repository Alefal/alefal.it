...
<form id="archives-filter" method="POST">
	...
	<?php if( !empty( $files ) ) { ?><a href="<?php echo esc_url( add_query_arg( array( 'action' => 'nuke_archives', '_wpnonce' => wp_create_nonce( 'woo_ce_nuke_archives' ) ) ) ); ?>" class="button action"><?php _e( 'Delete All', 'woocommerce-exporter' ); ?></a><?php } ?>
</form>