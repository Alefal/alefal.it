			...
			switch( $action ) {
				...
				case 'dismiss_checkout_addons_prompt':
					...
					if( !empty( $_GET ) && check_admin_referer( 'woo_ce_dismiss_checkout_addons_prompt' ) ) {
						...
						$url = array( add_query_arg( array( 'action' => null, '_wpnonce' => null ) ) );
						wp_redirect( $url );
						exit();
					}
					break;
				...
				case 'override_scheduled_export':
					...
					if( !empty( $_GET ) && check_admin_referer( 'woo_ce_override_scheduled_export' ) ) {
						...
						if( !empty( $scheduled_export ) ) {
							...
							$url = add_query_arg( array( 'tab' => 'settings', 'action' => null, 'post' => null, 'scheduled' => 1, '_wpnonce' => null ) );
							wp_redirect( $url );
							exit();
						}
					}
							...
							if( WOO_CD_DEBUG !== true ) {
								...
								if( woo_ce_get_option( 'delete_file', 1 ) ) {
									...
									if( $temp_filename == false ) {
										$message = sprintf( __( 'We could not create a temporary export file in <code>%s</code>, ensure that WordPress can read and write files here and try again.', 'woocommerce-exporter' ), apply_filters( 'woo_ce_sys_get_temp_dir', sys_get_temp_dir() ) );
										woo_cd_admin_notice( $message, 'error' );
