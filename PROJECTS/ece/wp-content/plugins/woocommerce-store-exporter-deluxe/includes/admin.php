...
<div id="woo-ce" class="wrap">
	...
	<h2>
		<?php echo $output; ?>
		<!-- <a href="<?php echo esc_url( add_query_arg( array( 'tab' => 'export', 'empty' => null ) ) ); ?>" class="add-new-h2"><?php _e( 'Add New', 'woocommerce-exporter' ); ?></a> -->
	</h2>
<?php
...
function woo_cd_add_settings_link( $links, $file ) {
	...
	if( $file == $this_plugin ) {
		...
		$support_link = sprintf( '<a href="%s" target="_blank">' . __( 'Premium Support', 'woocommerce-exporter' ) . '</a>', $support_url );
...
function woo_ce_admin_order_column_content( $column ) {
	...
	if( $column == 'woo_ce_export_status' ) {
		if( $is_exported = ( get_post_meta( $post->ID, '_woo_cd_exported', true ) ? true : false ) ) {
			...
			printf( '<mark title="%s" class="%s">%s</mark>', __( 'This Order has not yet been exported.', 'woocommerce-exporter' ), 'csv_not_exported', __( 'Not Exported', 'woocommerce-exporter' ) );
		}
...
function woo_ce_add_help_tab() {
	...
	$screen->add_help_tab( array(
		...
		'content' => 
			...
			'<p><a href="' . 'http://www.visser.com.au/documentation/store-exporter-deluxe/usage/' . '" target="_blank" class="button button-primary">' . __( 'Documentation', 'woocommerce-exporter' ) . '</a> <a href="' . 'http://www.visser.com.au/premium-support/' . '" target="_blank" class="button">' . __( 'Premium Support', 'woocommerce-exporter' ) . '</a></p>'
	) );
...
function woo_ce_admin_plugin_row() {
	...
	if( !class_exists( 'VL_Updater' ) ) {
		...
		$download_url = esc_url( 'http://updates.visser.com.au/downloads/visser-labs-updater.zip' );
		...
		$message = sprintf( __( '<a href="%s">Install the Visser Labs Updater Plugin</a> (or download and manually install it from <a href="%s">here</a>) to get automatic update notifications for your Visser Labs Plugins.', 'woocommerce-exporter' ), $install_url, $download_url );
		...
		if( $plugins = array_keys( get_plugins() ) ) {
			foreach( $plugins as $plugin ) {
				if( strpos( $plugin, 'visser-labs-updater.php' ) !== false ) {
					$message = sprintf( __( '<a href="%s">Activate the Visser Labs Updater Plugin</a> to get automatic update notifications for your Visser Labs Plugins.', 'woocommerce-exporter' ), esc_url( admin_url( $activate_url ) ) );
				}
			}
		}
		echo '</tr><tr class="plugin-update-tr"><td colspan="3" class="plugin-update colspanchange"><div class="update-message">' . $message . '</div></td></tr>';
...
function woo_ce_dashboard_setup() {
	...
	if( woo_ce_get_option( 'enable_auto', 0 ) == 1 && current_user_can( 'manage_options' ) ) {
		wp_add_dashboard_widget( 'woo_ce_scheduled_export_widget', __( 'Scheduled Exports', 'woocommerce-exporter' ), 'woo_ce_scheduled_export_widget', 'woo_ce_scheduled_export_widget_configure' );
		wp_add_dashboard_widget( 'woo_ce_recent_scheduled_export_widget', __( 'Recent Scheduled Exports', 'woocommerce-exporter' ), 'woo_ce_recent_scheduled_export_widget', 'woo_ce_recent_scheduled_export_widget_configure' );
	}
