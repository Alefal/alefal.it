...
*/
	...
	if( class_exists( 'WC_Tab_Manager' ) ) {
		$product_tabs = false;
/*
		$product_tabs = woo_ce_get_product_tabs();
		if( !empty( $product_tabs ) ) {
			foreach( $product_tabs as $product_tab ) {
				if( !empty( $product_tab ) ) {
					$fields[] = array(
						'name' => sprintf( 'product_tab_%s', sanitize_key( $product_tab->post_name ) ),
						'label' => sprintf( __( 'Product Tab: %s - %s', 'woocommerce-exporter' ), get_the_title( $product_tab->post_parent ), ucfirst( $product_tab->post_title ) ),
						'hover' => __( 'WooCommerce Tab Manager', 'woocommerce-exporter' )
					);
				}
			}
			unset( $product_tabs, $product_tab );
		}
*/
...
/*
	// @mod - Needs alot of love, JetPack Plus is huge
	...
	if( has_term( 'variable', $term_taxonomy, $product_id ) ) {
		...
		if( method_exists( $_product, 'get_variation_regular_price' ) && method_exists( $_product, 'get_variation_sale_price' ) ) {
			...
			if( $min_price == $max_price ) {
				$product->price = woo_ce_format_price( $min_price );
				$product->sale_price = woo_ce_format_price( $min_sale_price );
			} else {
				$product->price = sprintf( apply_filters( 'woo_ce_product_variable_price', '%s-%s' ), woo_ce_format_price( $min_price ), woo_ce_format_price( $max_price ) );
				$product->sale_price = sprintf( apply_filters( 'woo_ce_product_variable_sale_price', '%s-%s' ), woo_ce_format_price( $min_sale_price ), woo_ce_format_price( $max_sale_price ) );
			}
			unset( $min_price, $max_price, $min_sale_price, $max_sale_price );
		}
	} else {
...
/*
		...
		if( !empty( $product_tabs ) ) {
			foreach( $product_tabs as $product_tab ) {
				if( !empty( $product_tab ) )
					$product->{sprintf( 'product_tab_%s', $product_tab->post_name )} = $product_tab->post_content;
			}
			unset( $product_tabs, $product_tab );
		}
*/
