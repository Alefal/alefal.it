		...
		foreach( $scheduled_exports as $scheduled_export ) {
			...
			if( !wp_next_scheduled( $hook, $args ) && get_post_status( $scheduled_export ) == 'publish' ) {
				...
				switch( $auto_schedule ) {
					...
					case 'custom':
						$recurrence = 'woo_ce_auto_interval_' . $scheduled_export;
						break;
		...
		foreach( $scheduled_exports as $scheduled_export ) {
			...
			switch( $schedule ) {
				...
				case 'custom':
					...
					if( $interval ) {
						$schedules['woo_ce_auto_interval_' . $scheduled_export] = array(
	...
	if( empty( $type ) ) {
		...
		if( !in_array( $export->type, $export_types ) ) {
			...
			switch( $export->export_format ) {
				...
				default:
					woo_ce_error_log( sprintf( 'Warning: %s', __( 'An invalid export format was provided', 'woocommerce-exporter' ) ) );
					return;
					break;
								...
								if( $directory_response ) {
									...
									if( $connection_response ) {
										...
										if( @ftp_delete( $connection, $export->filename ) == false ) {
											if( !empty( $path ) )
												woo_ce_error_log( sprintf( '%s: Error: %s', $export->filename, sprintf( __( 'Could not delete failed FTP upload of %s to %s', 'woocommerce-exporter' ), $export->filename, $path ) ) );
											else
