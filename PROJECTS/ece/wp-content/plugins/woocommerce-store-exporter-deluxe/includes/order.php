...
function woo_ce_get_order_data( $order_id = 0, $export_type = 'order', $args = array(), $fields = array() ) {
	...
	if( $export_type == 'order' ) {
		...
		$order->purchase_subtotal = $order->order_excl_tax - $order->shipping_cost;
...
function woo_ce_get_order_line_item_assoc_refunds( $line_item_id = 0 ) {
	...
	$refund_items_sql = $wpdb->prepare( "SELECT order_itemmeta.`order_item_id` FROM `" . $wpdb->prefix . "woocommerce_order_items` as order_items, `" . $wpdb->prefix . "woocommerce_order_itemmeta` as order_itemmeta WHERE order_items.`order_item_id` = order_itemmeta.`order_item_id` AND order_items.`order_item_type` = 'line_item' AND order_itemmeta.`meta_key` = %s AND order_itemmeta.`meta_value` = %d", $meta_key, $line_item_id );
				...
				if( $order_item_meta = $wpdb->get_results( $order_item_meta_sql ) ) {
					...
					for( $i = 0; $i < $size; $i++ ) {
						...
						switch( $order_item_meta[$i]->meta_key ) {
							...
							// This is for Order Item meta we can ignore
							case '_line_tax_data':
								continue;
								break;
		...
		if( isset( $options['billing_buttons'] ) ) {
			...
			if( !empty( $buttons ) ) {
				foreach( $buttons as $button ) {
					...
					if( $button['type'] == 'heading' )
						continue;
					$order->{sprintf( 'shipping_%s', $button['cow'] )} = woo_ce_format_custom_meta( get_post_meta( $order_id, sprintf( '_billing_%s', $button['cow'] ), true ) );
				}
				unset( $buttons, $button );
			}
		}
